...
| `wizard-batch-test-job/` | **Lives on the `wizard-batch-demo` branch, not `main`.** Isolated fixture for the fusion-spectra "Git → Batch Job" wizard: same single-fixed-entrypoint shape as `batch-report-generator/`, but its own unique `name` so wizard runs never move that fixture's `"stable"` tag, and a dedicated branch so the wizard's repo-ref (branch) field is actually exercised end-to-end. Also carries `gitwatcher-payload.example.json` — a copy-pasteable `POST /api/forge/api/v1/gitwatchers` body for testing the same flow by hand, without going through the wizard UI. | Not wired into the Helm showroom — wizard-test-only, same convention as `app-builds/wizard-test-job/` (Git → Python Job wizard's fixture). |
