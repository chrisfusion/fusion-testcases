...
|---|---|---|
| `streamlit-showcase/` | Standard web app build: `runner.type: streamlit`, a port, an ingress `path`. Long-running service → deployed as a `Deploy`-kind step. | `showroom.codeSourceApps.streamlitShowcase` (`codesource-streamlit.yaml`) — artifact `app.streamlit-showcase` |
